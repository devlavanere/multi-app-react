...
const IPAddressFinder = () => {
  ...
  const findIP = async () => {
    ...
    setError(''); // Limpa os erros antes de fazer a requisição
    ...
    try {
      ...
      setError('Error fetching IP address data. Please try again.'); // Define uma mensagem de erro em caso de falha
      console.error("Error fetching IP address data:", error); // Exibe um erro no console para debug
      setIpData(null); // Limpa os dados anteriores caso haja um erro
    } finally {
  ...
  return (
    <Container>
      ...
      />
      <Button onClick={findIP} disabled={loading}> {/* Botão que chama a função findIP quando clicado */}
        {loading ? 'Loading...' : 'Find IP'}
      </Button>
      {error && <p style={{ color: 'red' }}>{error}</p>} {/* Exibe uma mensagem de erro se houver */}
